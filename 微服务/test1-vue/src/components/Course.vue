...
<!-- 微信支付二维码 -->
        <el-dialog :visible.sync="dialogFormVisible" :before-close="cancelOrder" :modal="true" :close-on-click-modal="false"
              ...
              <!-- 留言 开始 -->
                      <div class="message-list" v-for="(comment , index) in commentList" :key="index">
                        <div class="message-list-title">
                          ...
                          <!-- 已赞 -->
                          <div @click="cancelzan(comment)" v-if="JSON.stringify(comment.favoriteRecords).indexOf(userId) >= 0" class="message-list-title-right">
...
this.$nextTick(function(){
  this.createCode();  // 直接调用会报错
    ...
    createCode(){
      // 去获取支付连接
      this.axios.get("http://localhost:8006/pay/createCode",{
        params:{
...
// 真实的修改数据库，保存订单！
    ...
    updateOrder(statusCode){
      return this.axios.get("http://localhost:8007/order/updateOrder",{
        params:{
        orderNo:this.orderNo,
        status:statusCode,
        user_id: this.userId,
   ...
   // 取消订单（支付前，关闭二维码窗口视为不购买，可取消订单）
    ...
    // 获取本课程的全部留言
    getComment(){
      return this.axios.get("http://localhost:8005/comment/getCourseCommentList/"+this.course.id+"/1/20").then((res) => {
    ...
    return this.axios.get("http://localhost:8004/course/getCoursesByUserId/"+this.userId).then((res) => {
        console.log(res);
  ...
  // 发表留言
  saveComment(){  
    return this.axios.get("http://localhost:8005/comment/saveCourseComment",{
      params:{
        courseId:this.course.id,
        userId:this.userId,
    ...
    zan(comment){     
      ...
      if(this.isLogin === true){
        return this.axios.get("http://localhost:8005/comment/saveFavorite/"+ comment.id +"/"+this.userId).then((res) => {
       // 重新获取本门课的全部留言信息
       this.getComment();
      }).catch((err) => {
        ...
        this.$message.warning("请先登录，再来点赞吧！");
      }
    },
    // 取消赞
    cancelzan(comment){
      return this.axios.get("http://localhost:8005/comment/cancelFavorite/"+ comment.id +"/"+this.userId).then((res) => {
       // 重新获取本门课的全部留言信息
       this.getComment();
      }).catch((err) => {
